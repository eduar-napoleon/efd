const waitTillHTMLRendered = async (page, timeout = 30000) => {
  ...
  const urld = process.argv[6];
  var url = require('url');

  var base = url.parse(urld);
  ...
  (async() => {
    const browser = await puppeteer.launch({
      ...
      ,userDataDir: '/odoo/'+md5(base+user)
    ...
    try {
      ...
      var body = "{\"jsonrpc\":\"2.0\",\"method\":\"call\",\"params\":{\"model\":\"pos.order\",\"fields\":[\"name\",\"pos_reference\",\"partner_id\",\"date_order\",\"user_id\",\"amount_total\",\"company_id\",\"state\",\"session_id\"],\"domain\":[],\"context\":{\"lang\":\"en_US\",\"tz\":false,\"uid\":38,\"params\":{\"action\":302,\"min\":1,\"limit\":1000,\"view_type\":\"list\",\"model\":\"pos.order\",\"menu_id\":189,\"_push_me\":false},\"bin_size\":true},\"offset\":0,\"limit\":1000,\"sort\":\"\"},\"id\":90771387}";
      // console.log(body);
      ...
      // console.log(body); 
      // return;
      const output = await page.evaluate((data, base) => {
        return fetch(base+"/web/dataset/search_read", {
          "headers": {
